# 智慧醫療大數據 (Big Data for Intelligent Medical)

> 本專案提供智慧醫療大數據課程的教材與範例檔案，包含課堂規定、PPT 以及相關練習資源。

## 📚 目錄

- [智慧醫療大數據 (Big Data for Intelligent Medical)](#智慧醫療大數據-big-data-for-intelligent-medical)
  - [📚 目錄](#-目錄)
  - [📝 課堂規定、評量標準及課程進度安排](#-課堂規定評量標準及課程進度安排)
  - [📁 附件](#-附件)
  - [🧠 第一週：Overview - 智慧醫療大數據](#-第一週overview---智慧醫療大數據)
  - [🧠 第四週：大數據資料擷取收集整理 - Data Acquisition](#-第四週大數據資料擷取收集整理---data-acquisition)
  - [🧠 第五週：資料前處理 - Data Preprocessing](#-第五週資料前處理---data-preprocessing)
  - [🧠 第七-九週：以Kaggle3個Project為例](#-第七-九週以kaggle3個project為例)
    - [🔧 注意事項](#-注意事項)

---

## 📝 課堂規定、評量標準及課程進度安排

- [下載 PPT](0.課堂規定及評量標準.pptx)

## 📁 附件

- [Tkinter 使用範例 - Excel](Copy/Tkinter/UsingList.7z)
- [Tkinter 使用範例 - Sqlite](Copy/Tkinter/UsingSqlite.7z)

---

## 🧠 第一週：Overview - 智慧醫療大數據

- [中文投影片](1.第一週Overview_智慧醫療大數據_BigDataForIntelligence_Medicine.ppt)
- [英文補充教材：概念介紹](Copy/2.Concept_of_Big_Data_Analysis.pptx)
- [訓練-驗證-測試的資料_需Random切分_但有時間順序的例外](當使用的資料是有時間前後順序時_訓練-驗證-測試的資料_不可以Random切分.pdf)

---

## 🧠 第四週：大數據資料擷取收集整理 - Data Acquisition

- [第四週-1.大數據資料擷取收集整理.pdf](第四週-1.大數據資料擷取收集整理.pdf)
- [第四週-2.Python爬蟲教學.pdf](第四週-2.Python爬蟲教學.pdf)
- [第四週-2.Python爬蟲教學-程式.py](第四週-2.Python爬蟲教學-程式.py)
- [第四週-3.爬蟲404錯誤解決.pdf](第四週-3.爬蟲404錯誤解決.pdf)
- [第四週-4_ETLvsELT解析](第四週-4_ETLvsELT解析.pdf)

---

## 🧠 第五週：資料前處理 - Data Preprocessing

* [第5週-大數據前處理與範例.pdf](第5週-大數據前處理與範例.pdf)
* [第5週-微調上述部份程式](第5週-微調部份程式.7z)

---

## 🧠 第七-九週：以Kaggle3個Project為例

* 詳資料夾: Kaggle/1_InsuranceFraudDetection-Using12Models
* 詳資料夾: Kaggle/2_VehicleInsuranceFraudDetection
* 詳資料夾: Kaggle/3_InvestmentandFinancialAnalysis

---

#### 🔧 注意事項

請根據需要下載相應的檔案或範例，課程資源持續更新。
